# DASC7606 Final - Natural Language Processing - summarized by hyperloop
...
- **优势（Advantages）**
  ...
  - **“分布式”含义**：一个维度（一个位置的值）可以参与表示很多词；多个维度共同编码词的含义→ 能捕捉不同词之间的共性与差异
...
- **定义**：把词表示（word embeddings）放进**下游任务**的完整系统里，通过任务表现来评估词向量质量流程：Input → **Words to word vectors** → **Deep Learning system** → answer
- **典型下游任务示例**
